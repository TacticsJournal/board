...
test('hosted mode keeps official and self-hosted extensions separate', () => {
  ...
  assert.match(main, /skillsSectionUnavailable">/)
  assert.match(main, /<div class="setGroup" aria-disabled="true">\s*<div class="setItem setItemStatic">/)
  assert.match(main, /<span class="setItemValue">Self-hosted only<\/span>/)
  assert.match(main, /<span class="setItemValue">Self-hosted only<\/span>\s*<\/div>\s*<\/div>\s*<p class="setNote"><a class="skillsSelfHostLink" href="https:\/\/github\.com\/TacticsJournal\/board" target="_blank" rel="noopener noreferrer">Instructions to self-host are on Github<\/a><\/p>/)
