...
test('the share button owns copy links, invitations, and export', () => {
  ...
  assert.match(main, /createBoardImportUrl\(boardLinkName, newProjectFromBoard\(source, boardLinkName\)\)/)
  assert.match(main, /createProjectImportUrl\(projectName, projectCopy\)/)
  assert.match(main, /The link does not expire\./)
  assert.match(main, /function addSharedProject\(draft: AgentImportDraft\)/)
  assert.match(main, /canCreateProject\(library, entitlements\.hasPaidBoardAccess\(\)\)/)
  assert.match(main, /source\.boards\.slice\(1\)\.forEach\(item => appendBoardCopy\(project, item\)\)/)
  assert.match(main, /addProject: addSharedProject/)
  assert.doesNotMatch(main, /function boardLinkPreview\(/)
